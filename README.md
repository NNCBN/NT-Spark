...
Boot nodes are required for the Nervos Network to operate. These nodes maintain connections to other reachable network nodes. When a new node initiates synchronization for the first time, it would not know where to find full nodes for synchronization without specified boot nodes. These very boot nodes are currently operated in data centers belonging to large corporations. Those lack diversity and are subject to legal and economic influences. Creation of a new boot node that is operated not by a company and not hosted in a data center, but regionally by an individual. Support for interested parties who wish to promote the decentralization of the NervosNetwork by operating a boot node themselves. 
- Ecosystem Relevance:
