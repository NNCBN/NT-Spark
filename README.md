...
Create and publish a curated list of hand-selected boot nodes that are used at random intervals for initial synchronization tests. Boot nodes must be constantly accessible so that nodes can be initially synchronized and connected through them. Preference will be given to geographically underrepresented countries as well as operators that maintain a diverse operating system platform. Bootnodes that rely on data centers or VPN operators are not of interest here.
...
https://nodes.ckb.dev lists permanently operational nodes. This one will appear as a dot in the Atlantic Ocean west of Senegal. Performance monitoring is also documented and published.
